...
router.get('/callback', catchAsyncErrors(async (req, res) => {
    ...
    axios.post('https://discord.com/api/oauth2/token', data, {
        ...
        if (valid) {
            ...
            await DiscordClient.sendMessage(config.discord.logChannelId, `${user.id} Authenticated successfully.`);
            ...
            await DiscordClient.sendMessage(config.discord.logChannelId, `${user.id} Not authorized to access map.`);
            res.redirect('/login');
        }
    }).catch(error => {
        console.error(error);
